...
)
{
  ...
  {
    if (i != 0)
      sqlite3_str_appendf(pStr, "%s", zSep);
...
static void rbudiff_one_table(const char *zTab, FILE *out)
{
  ...
  ** columns are found, bail out early.  */
  ...
  if (bOtaRowid)
    sqlite3_str_appendf(pCt, "rbu_rowid, ");
  ...
  ** in the data_xxx table. */
  ...
  if (bOtaRowid)
    sqlite3_str_appendf(pInsert, "rbu_rowid, ");
      ...
      for (i = 0; i < nCol; i++)
      {
        ...
        if (i >= nPK && sqlite3_column_type(pStmt, i) == SQLITE_BLOB && sqlite3_column_type(pStmt, nCol + 1 + i) == SQLITE_BLOB)
        {
          ...
          if (nDelta < nFinal)
          {
            ...
            sqlite3_fprintf(out, "x'");
            for (j = 0; j < nDelta; j++)
              sqlite3_fprintf(out, "%02x", (u8)aDelta[j]);
            sqlite3_fprintf(out, "'");
    ...
    int nVal, sqlite3_value **apVal)
{
  ...
  if (zSql == 0 || sqlite3_stricmp(zToken, "create"))
    return;
  zSql = gobble_token(zSql, zToken, sizeof(zToken));
  if (zSql == 0 || sqlite3_stricmp(zToken, "virtual"))
    return;
  zSql = gobble_token(zSql, zToken, sizeof(zToken));
  if (zSql == 0 || sqlite3_stricmp(zToken, "table"))
    return;
  zSql = gobble_token(zSql, zToken, sizeof(zToken));
  if (zSql == 0)
    return;
  zSql = gobble_token(zSql, zToken, sizeof(zToken));
  if (zSql == 0 || sqlite3_stricmp(zToken, "using"))
    return;
  zSql = gobble_token(zSql, zToken, sizeof(zToken));
  ...
  for (i = 1; i < argc; i++)
  {
    ...
    if (z[0] == '-')
    {
      ...
      if (strcmp(z, "changeset") == 0)
      {
        ...
        if (out == 0)
          cmdlineError("cannot open: %s", argv[i]);
